...
def hybrid_retrieve_with_rerank(query, fusion_top_k=5, final_top_k=3):
    # Step 1: hybrid fusion (same as before) - get top 5 candidates
    ...
    # Step 2: rerank those candidates with the cross-encoder
    pairs = [[query, chunk] for chunk in candidate_chunks]
    rerank_scores = reranker.predict(pairs)

    # Step 3: sort by rerank score, keep final_top_k
...
def hybrid_rerank_rag_pipeline(query):
    chunks = hybrid_retrieve_with_rerank(query)
    answer = generate_answer(query, chunks)
    print(f"\nQuestion: {query}")
    print(f"Answer: {answer}")

# --- Test it ---
hybrid_rerank_rag_pipeline("When did Beyonce start becoming popular?")
hybrid_rerank_rag_pipeline("What is solar energy?")
hybrid_rerank_rag_pipeline("Who wrote To Kill a Mockingbird?")
hybrid_rerank_rag_pipeline("What is the capital of France?")